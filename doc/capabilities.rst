...
Python has something of an undocumented singleton in :func:`curses.setupterm`: it allows only a
...
If you find any entry missing, please open an issue `on GitHub
<https://github.com/Rockhopper-Technologies/jinxed/issues>`_ or create a pull request for
`jinxed/terminals.toml <https://github.com/Rockhopper-Technologies/jinxed/blob/main/terminals.toml>`_
...
- All ``\x1b(B``, ``\x1b(0``, ``\x1b(A``, ``\x1b(U``, ``\x1b(K`` G0/G1 Set Graphic Set designations
  ...
  Historically, when a curses program used an sgr sequence the final parameter could be used to
  enable line drawing mode, ``tparm(sgr, 0,0,0,0,0,0,0,0,1)``, and the sgr string was yielded with
  ``\x1b(0``, and calling with param 9=0 (default) yields with ``\x1b(B`` to switch back.
...
- The ``smacs``, ``rmacs``, ``enacs``, ``s0ds``, and ``s1ds`` capabilities are dropped entirely.
  ...
  via G0/G1 designation or Shift-Out/Shift-In.
